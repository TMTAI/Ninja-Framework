...
public class StoryUtility {
    ...
    public List<ExecutionIssueVO> findAllTestExecutionInStory(JQLIssueVO issue, Map<String, String> cookies) throws APIException {
    	return findAllTestExecutionInStory(issue, cookies, false);
    }
    
    public List<ExecutionIssueVO> findAllTestExecutionInStory(JQLIssueVO issue, Map<String, String> cookies, boolean retrieveCycles) throws APIException {
        List<ExecutionIssueVO> result = new ArrayList<>();
        logger.fastDebug("findAllTestExecutionInStory issue type %s:%s", issue.getKey(), issue.getFields().getIssuetype().getName());
        if (JQLIssuetypeVO.Type.STORY.toString().equalsIgnoreCase(issue.getFields().getIssuetype().getName())) {
            ...
            logger.fastDebug("issueLinks issue %s:%s", issue.getKey(), issueLinks.size());
            if (issueLinks != null && !issueLinks.isEmpty()) {
                for (JQLIssueLinkVO issueLink : issueLinks) {
                    List<ExecutionIssueVO> executionIssues = EpicUtility.getInstance().findTestExecutionInIsuee(issueLink.getInwardIssue().getKey(), cookies, retrieveCycles);
    ...
    public List<JQLIssueLinkVO> findAllTestIssueForStory(JQLIssueVO issue) {
    	logger.fastDebug("findAllTestIssueForStory issue %s", issue.getKey());
        ...
        if (issue != null && issue.getFields() != null && issue.getFields().getIssuelinks() != null) {
            ...
            testIssue = issueLinks.stream().filter(i -> (INWARD_TEST_BY.equals(i.getType().getInward()) || IS_TESED_BY.equals(i.getType().getInward()))).collect(Collectors.toList());
        } else {
