...
//        issues.addAll(findAllIssuesInEpicLink(epic, cookies));
    ...
    public List<ExecutionIssueVO> findTestExecutionInIsuee(String issueKey, Map<String, String> cookies) throws APIException {
    	return findTestExecutionInIsuee(issueKey, cookies, false);
    }

    public List<ExecutionIssueVO> findTestExecutionInIsuee(String issueKey, Map<String, String> cookies, boolean retrieveCycles) throws APIException {
    	logger.fastDebug("findTestExecutionInIsuee issue %s", issueKey);
        ...
        if (executions != null) {
            if (executions.getExecutions() != null) {
            	logger.fastDebug("findTestExecutionInIsuee executions %s:%s", executions.getExecutions().size(), issueKey);
                if (executions.getExecutions().size() > 1) {
                     ...
                     */
                    Optional<ExecutionIssueVO> execution = executions.getExecutions().stream().filter(i -> "".equals(i.getExecutedOn())).findFirst();
                    if (execution.isPresent() && execution.get() != null) {
                        testExecution.add(execution.get());
                    } else if(!retrieveCycles) {
                    	     execution = executions.getExecutions().stream().sorted(new Comparator<ExecutionIssueVO>() {
                             @Override
                ...
                } else if(executions.getExecutions().size() > 0) {
                	if(!retrieveCycles || "".equals(executions.getExecutions().get(0).getExecutedOn())) {
