            ...
            testExecution.forEach(new Consumer<ExecutionIssueVO>() {
                ...
                public void accept(ExecutionIssueVO issue) {
                    switch (issue.getStatus().getName()) {
                        case "PASS":
                        	if(gadgetData.getPassed().getIssues().add(issue.getIssueKey())) {
                        		gadgetData.increasePassed(1);
                        	}
                            break;
                        case "FAIL":
                        	logger.info("Increasing Failed " + gadgetData.getFailed().getTotal());
                        	if(gadgetData.getFailed().getIssues().add(issue.getIssueKey())) {
                        		gadgetData.increaseFailed(1);
                        	}
                            break;
                        case "UNEXECUTED":
                        	logger.info("Increasing UNEXECUTED " + gadgetData.getUnexecuted().getTotal());
                        	if(gadgetData.getUnexecuted().getIssues().add(issue.getIssueKey())) {
                        		gadgetData.increaseUnexecuted(1);
                        	}
                            break;
                        case "WIP":
                        	if(gadgetData.getWip().getIssues().add(issue.getIssueKey())) {
                        		gadgetData.increaseWip(1);
                        	}
                            break;
                        case "BLOCKED":
                        	if(gadgetData.getBlocked().getIssues().add(issue.getIssueKey())) {
