var app = angular.module('App', ['ngAnimate', 'ngMaterial', 'ngResource', 'ngMessages', 'ngCookies', 'md.data.table']).filter('roundup', function () {
...
app.run(function ($rootScope, $resource, $location, $cookies, $mdToast, $mdMenu, $mdDialog) {
    ...
    $rootScope.showAdminFilterDialog = function () {
        $mdDialog.show(
            $mdDialog.alert()
                .clickOutsideToClose(true)
                .title('This is an administrator function')
                .textContent('Only administrator user can use this function.')
...
app.controller('HomePageCtrl', function ($rootScope, $scope, $resource, $mdDialog, $mdToast, $cookies, $location, $filter) {
    ...
    $rootScope.getDashboardList = function () {
        ...
        $resource('/getDashboardList', {
            ...
            $scope.selectedDashboardId = $rootScope.currentDashboard.id;
...
app.controller('EpgDashboardConfigCtrl', function ($rootScope, $scope, $mdDialog, $mdToast, $location, $resource, $cookies) {
    $rootScope.pageName = "EpgDashboardConfiguration";
    $rootScope.userInfo = $cookies.getObject("userInfo");
    $rootScope.userfullname = $rootScope.userInfo.displayName;
    $scope.ReleaseList = [];
    $scope.newReleaseName = "";
    $scope.newReleaseUrl = "";
    var getData = $resource('/getAllReleaseWithUrl', {},
        {
            query: {
                method: 'get',
                isArray: true
    ...
    var getGreenHopperReleaseList = function () {
        getData.query().$promise.then(function (data) {
            $scope.ReleaseList = data;
        }, function (error) {
            console.log(error);
        });
    };
    getGreenHopperReleaseList();
    // $scope.getReleaseList = function () {
    //     $rootScope.getReleaseRs.query().$promise.then(function (data) {
    //         $scope.ReleaseList = data;
    //     }, function (error) {
    ...
    //     {
    //         "name": "1.2.0",
    //         "url": "https://greenhopper.app.alcatel-lucent.com/sr/jira.issueviews:searchrequest-xml/temp/SearchRequest.xml?jqlQuery=project+%3D+FNMS+AND+issuetype+in+%28Epic%2C+Story%29+AND+fixVersion+%3D+1.2.0&tempMax=10000"
    //     },
    //     {
    //         "name": "1.2.0",
    //         "url": "https://greenhopper.app.alcatel-lucent.com/sr/jira.issueviews:searchrequest-xml/temp/SearchRequest.xml?jqlQuery=project+%3D+FNMS+AND+issuetype+in+%28Epic%2C+Story%29+AND+fixVersion+%3D+1.2.0&tempMax=10000"
    ...
    $scope.addNewRelease = function (ev) {
        if ($scope.newReleaseName.length == 0 || $scope.newReleaseUrl.length == 0) {
            ...
            $scope.ReleaseList.push({
                'name': $scope.newReleaseName,
                'url': $scope.newReleaseUrl,
                'display': $scope.newReleaseDisplay
            });
            $resource('/addNewGreenHopperRelease', {
                name: $scope.newReleaseName,
                url: $scope.newReleaseUrl,
                display: $scope.newReleaseDisplay
            }).save().$promise.then(function (data) {
    ...
    $scope.deleteRelease = function (release, ev) {
        ...
        $mdDialog.show(confirm).then(function () {
            ...
            if (index > -1) {
                $scope.ReleaseList.splice(index, 1);
                $resource('/deleteGreenHopperRelease', {
                    url: release.url
                }).save().$promise.then(function (data) {
                    console.log(data)
                }, function (error) {
    ...
    $scope.editRelease = function (release, ev) {
        if (typeof release.name == 'undefined' || typeof release.url == 'undefined') {
            ...
            $resource('/updateGreenHopperRelease', {
                id: release.id,
                name: release.name,
                url: release.url,
...
app.controller('EpgDashboardCtrl', function ($rootScope, $scope, $mdDialog, $mdToast, $location, $resource, $cookies, $http, $parse, $interval, $anchorScroll, $document, $window) {
    $anchorScroll.yOffset = 20;
    $rootScope.pageName = "EPG Dashboard";
    $rootScope.userInfo = $cookies.getObject("userInfo");
    $rootScope.userfullname = $rootScope.userInfo.displayName;
    $scope.data = null;
    $scope.tabList = null;
    $scope.storytable = [];
    $scope.epictable = [];
    $scope.message = 'Loading GreenHopper data ... Please do not change to another tab!';
    $scope.updateTime = null;
    $scope.updateInfo = null;
    $scope.gadgetAge = Date.now();
    $scope.titleAdditionalInfo = null;
    $scope.showView = true;
    $scope.isClearingCache = false;
    $scope.currentTabSelectedName = null;
    $scope.orderByField = 'key';
    $scope.reverseSort = false;
    $scope.newColumn = [];
    $scope.columnSpan = function(){
        var count = 0;
        var length = $scope.allEpicDetailsColumns.length;
        for (var i = 0; i < length; i++) {
            if ($scope.allEpicDetailsColumns[i].show == true) {
                count++;
            }
        }
        return count;
    ...
    var columnConfiguration = $resource('/EpgDashboard/columnConfiguration', {},
        {
            save: {
                method: 'post'
            },
    ...
    $scope.allEpicDetailsColumns = columnConfiguration.get().$promise.then(function (response){
        var result = response.configuration;
        if (result != undefined) {
            $scope.allEpicDetailsColumns = result;
        } else {
            $scope.allEpicDetailsColumns = defaultConfiguration;
        }
    }, function (error){
        $scope.allEpicDetailsColumns = defaultConfiguration;
    });
    ...
    $scope.addNewColumn = function(){
        $mdDialog.show({
            ...
            scope: $scope,
            preserveScope: true,
            templateUrl: 'assets/html/addNewColumn.tmpl.html',
            controller:
                function DialogController($scope, $mdDialog) {
                    $scope.cancel = function() {
                        $mdDialog.hide();
                    ...
                    $scope.save = function(){
                        $mdDialog.hide();
                        columnConfiguration.save($scope.allEpicDetailsColumns);
                    }
                }
        });
    };

    $scope.doSorting = function (criteria){
        $scope.orderByField = criteria;
    ...
    $scope.getListRelease = function () {
        getData.query().$promise.then(function (data) {
            $scope.tabList = data;
            $scope.currentTabSelectedName = $scope.tabList[0].name;
            $scope.onTabSelected($scope.currentTabSelectedName, false);
        }, function (error) {
            //console.log(error);
            // console.log(error);
        });
    ...
    $scope.calculateRatio = function (count, total) {
        return Math.floor((count / total) * 100);
    };

    $scope.getEpicDetail = function () {
        $scope.promise = $scope.epicsDetail.data.$promise;
    };
    // var urlTest = 'https://greenhopper.app.alcatel-lucent.com/sr/jira.issueviews:searchrequest-xml/temp/SearchRequest.xml?jqlQuery=project+%3D+FNMS+AND+issuetype+in+%28Epic%2C+Story%2C+Sub-task%29+AND+fixVersion+%3D+1.2.0&tempMax=10000';

    $scope.onTabSelected = function (name, updateCache) {
        if (!$scope.showProgressCircular) {
            $scope.data = [];
            var getData;
            if (updateCache) {
                getData = $resource(
                    '/EpgDashboard/updateCacheData',
                    {name: name},
                    {
                        query: {method: 'get'}
                    });
            ...
            getData.query().$promise.then(function (response) {
                if (response.cacheData!='updating') {
                    $scope.data = JSON.parse(response.cacheData).data;
                    $scope.showProgressCircular = false;
                    $scope.updatingData = false;
                    $scope.updateTime = 0;
                    var latestUpdate = new Date(response.updateDate);
                    $scope.updateInfo = "Latest update time: " + latestUpdate.toString();
                    $scope.storytable = [];
                    $scope.epictable = [];
                    for (var key in $scope.data.listProduct) {
                        $scope.epictable.push({
                            query: {
                                order: 'key',
                                limit: 10,
                                page: 1
    ...
    // $scope.init($scope.tabList[0].url);


    $scope.getProgressBarDefaultValue = function (number) {
        if (number !== number) {
            return 0;
        } else {
            return number;
    ...
    $scope.getColor = function (colorParam) {

        if (colorParam == 'LA Needed') {
            return '#FF6F83';//red
        } else if (colorParam == 'Ready for Selection') {
            return '#FF6F83';//red
        } else if (colorParam == 'Selected') {
            return '#FFA351';//orange
        } else if (colorParam == 'Not Created') {
            return '#FF0149';//red
        } else if (colorParam == 'To Do') {
            return '#FF6F83';//red
        } else if (colorParam == 'todo') {
            return '#FF6F83';//red
        } else if (colorParam == 'Defined') {
            return '#FFD351';//yellow
        } else if (colorParam == 'In Progress') {
            return '#EEEEEE';//grey
        } else if (colorParam == 'In Review') {
            return '#C5FFC5';//green
        } else if (colorParam == 'Done') {
            return '#8BFF8B';//green
        } else {
            return '#FFFFFF';
            ;
    ...
    //refresh data for each release
    $scope.clearCacheEpgDashboard = function () {
        $scope.onTabSelected($scope.currentTabSelectedName, true);
    };

    // Set interval to load data from db auto after every 15 minutes
    ...
    //Get list component by product name
    $scope.getListComponentByProductName = function (productName) {
        var listComponentNane = [];
        angular.forEach($scope.data.epicList, function (epicValue, key) {
            if (epicValue.product == productName) {
                if (listComponentNane.indexOf(epicValue.component) == -1) {
                    listComponentNane.push(epicValue.component);
                }
            }
            ;
        });
        return listComponentNane;
    };

    $scope.gotoAnchor = function (anchorId, isEpic, isTableOfContent) {
        var newHash = '';
        if (isTableOfContent) {//if it is table of content
            $anchorScroll.yOffset = 20;
            if (isEpic) {
                newHash = 'EpicDetail.' + anchorId;
            } else {
                newHash = 'SummaryDetail.' + anchorId;
            }
        } else {
            $anchorScroll.yOffset = -20;
            newHash = newHash + anchorId;
        }
    ...
    $scope.getAllEpicOfProduct = function (data) {
        var listAllEpic = [];
        angular.forEach(data, function (value, key) {
            listAllEpic = listAllEpic.concat(value);
        });
        return listAllEpic;
    };

    $scope.calculatePercentageEpic = function (epic) {
        var sum = epic.userStoryCounter.defined * 25 + epic.userStoryCounter.inProgress * 25 + epic.userStoryCounter.inReview * 50 + epic.userStoryCounter.done * 100;
        var total = epic.userStoryVOS.length;
        if (total == 0)
            return 0;
        else
            return sum / total;
    ...
    $scope.calculateProgress = function (userStoryCounter){
        var sum = userStoryCounter.userStoryCount.defined * 25 + userStoryCounter.userStoryCount.inProgress * 25 + userStoryCounter.userStoryCount.inReview * 50 + userStoryCounter.userStoryCount.done * 100;
        var total = userStoryCounter.usTotal;
        if (total == 0)
            return 0;
        else
    ...
    $scope.calculateSubTaskPercentage = function (story) {
        var subTaskCounterResult = story.subTasksCounter.subTaskCountResults;
        if (subTaskCounterResult != undefined) {
            var sum = subTaskCounterResult.inProgress * 25 + subTaskCounterResult.inReview * 50 + subTaskCounterResult.done * 100;
            var total = subTaskCounterResult.open + subTaskCounterResult.inProgress + subTaskCounterResult.inReview + subTaskCounterResult.done;
            if (total == 0)
                return 0;
            else
                return Math.floor(sum / total);
    ...
    $scope.showPercentageSubTaskColumn = function (story) {
        var subTaskCounterResult = story.subTasksCounter.subTaskCountResults;
        var sum =subTaskCounterResult.open + subTaskCounterResult.inProgress + subTaskCounterResult.inReview + subTaskCounterResult.done;
        if (sum != 0) {
            return true;
        } else  return false;
    }
