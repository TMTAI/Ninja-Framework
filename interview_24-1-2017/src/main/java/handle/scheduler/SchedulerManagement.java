...
public class SchedulerManagement {
    ...
    private SchedulerManagement() {
        try {
            ...
            startClearJob();
            startUpdateGreenHopperCacheData();
            scheduler.start();
    ...
    private void startUpdateGreenHopperCacheData() {
        int intervalInSeconds = 900;
        JobDetail job = JobBuilder.newJob(GreenHopperReleaseDataJob.class).withIdentity("UPDATE_GREENHOPPER_DATA_CACHE", "API").build();
        Trigger trigger = TriggerBuilder.newTrigger().withIdentity("UPDATE_GREENHOPPER_DATA_CACHE", "API")
                .startNow().withSchedule(SimpleScheduleBuilder.simpleSchedule().withIntervalInSeconds(intervalInSeconds)
                        .repeatForever()).build();
        try {
            scheduler.scheduleJob(job, trigger);
            logger.info("started Update Greenhopper data cache job");
        } catch (SchedulerException e) {
            logger.error("Cannot Update Greenhopper data cache job", e);
        }
